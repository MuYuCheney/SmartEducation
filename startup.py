...
def create_openai_api_app(
    ...
    """
    创建一个 FastAPI 应用，该应用充当一个 OpenAI API 服务器。它配置了 CORS 中间件、日志记录器，并根据提供的参数设置了控制器地址和 API 密钥
    ...
    # 添加 CORS（跨源资源共享）中间件，允许所有来源、方法和头部，支持凭据
    app.add_middleware(
        CORSMiddleware,
...
def run_controller(log_level: str = "INFO", started_event: mp.Event = None):
    """
    启动一个 FastAPI 应用。这个应用有一个接口可以释放和加载model worker，为下文的/release_worker
    :param log_level:
    :param started_event:
    :return: APP实例
    """
    ...
    # 定义一个 POST 接口 /release_worker，用于释放 model Worker
...
def dump_server_info(after_start=False, args=None):
    ...
    # 读取加载的模型，如果未在命令行进行指定，会加载默认的模型
    # 位置在：/home/rag/Langchain-Chatchat/configs/model_config.py：LLM_MODELS = ["chatglm3-6b", "zhipu-api", "openai-api"]
    models = LLM_MODELS
    if args and args.model_name:
        models = args.model_name

    # 默认使用的是 ChineseRecursiveTextSplitter，位置：/home/rag/Langchain-Chatchat/configs/kb_config.py
...
async def start_main_server():
    ...
    # 由 Ctrl+C 触发的，用于请求中断程序（如终止一个正在运行的命令行程序）
    signal.signal(signal.SIGINT, handler("SIGINT"))

    # SIGTERM 信号是一个终止信号，通常用于请求程序正常退出
    signal.signal(signal.SIGTERM, handler("SIGTERM"))

    # 设置 Python 多进程的启动方法。使用 "spawn" 方法时，每当程序创建一个新进程，操作系统会启动一个全新的 Python 解释器进程，然后运行这个新进程
...
if __name__ == "__main__":
    ...
    # 初始化定义的关系型数据库的表，存储在/home/rag/Langchain-Chatchat/server/db/models下
    create_tables()

    # 根据Python的版本，来调用不同的事件循环来执行异步操作。
